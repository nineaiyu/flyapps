...
def reply_login_msg(rec_msg, to_user, from_user):
    ...
    u_data_id = -1
    ids = []
    wx_user_obj_queryset = ThirdWeChatUserInfo.objects.filter(weixin__openid=to_user, enable_login=True).all()
...
def wx_bind_utils(rec_msg, to_user, from_user, content):
    ...
    wx_user_obj = ThirdWeChatUserInfo.objects.filter(weixin__openid=to_user, user_id__uid=uid).first()
            ...
            if isinstance(rec_msg, receive.Msg):
                ...
                if rec_msg.Event == 'CLICK':  # 公众号点击事件
                    if rec_msg.Eventkey == 'good':
                        ...
                        wx_user_obj_queryset = ThirdWeChatUserInfo.objects.filter(weixin__openid=to_user).filter(
    ...
    def get(self, request):
        ...
        if get_login_type().get('third', '').get('wxp'):
            ...
            info_serializer = page_obj.paginate_queryset(queryset=wx_obj_lists.order_by("-subscribe_time"),
                                                         request=request,
                                                         view=self)
            wx_user_info = WeixinInfoSerializer(info_serializer, many=True, )
